...
[Vanderlinde dos Santos (2019)](#citation) on repetitive head impacts and
...
## Protecting this repository (for the maintainer)

The repository is public so anyone can read and learn from it, but only the
maintainer can change the canonical version. Public ≠ editable. Here's how
to make sure of that.

### What's safe by default

- **Forks.** Anyone can fork (make a copy in their own GitHub account). Their
  fork is independent of yours; their changes do not propagate back. Forks
  are normal and welcomed in open-source.
- **Pull requests.** Anyone can propose changes via PR, but PRs only land in
  *your* repo when *you* click "Merge".
- **Read-only access.** The default for non-collaborators is read-only.

### What you should configure once on GitHub

After pushing to GitHub, go to your repo's **Settings** and enable:

1. **Settings → Branches → Add branch protection rule** for `main`:
   - ☑ Require a pull request before merging
   - ☑ Require approvals (set to 1)
   - ☑ Do not allow bypassing the above settings
   - ☑ Restrict who can push to matching branches *(empty list = nobody can push directly, must use PR)*
   - ☑ Allow force pushes: **Off**
   - ☑ Allow deletions: **Off**

2. **Settings → Collaborators and teams**:
   - Only add people you actively want pushing changes
   - Use the "Triage" or "Read" role for collaborators who don't need write
   - Remove old students/lab members when they leave

3. **Settings → Code security and analysis**:
   - ☑ Dependabot alerts — notifies you of vulnerable dependencies
   - ☑ Dependabot security updates — auto-creates PRs to patch them

4. **Tag stable releases**. After a major change is tested and working:

   ```bash
   git tag -a v0.2.0 -m "v0.2.0: both-axis FRF, summary metrics, Hwang plots"
   git push origin v0.2.0
   ```

   Users can then pin to a specific version (e.g., when downloading the ZIP
   from GitHub, they can choose a specific tag rather than the bleeding-edge
   `main` branch).

5. **Add a `CODEOWNERS` file** to require your review on changes:
...
Don't worry about people forking your repo and using or modifying it in
their own copies. That's the entire point of open source — your work gets
used. Their fork is *theirs*, not yours, and your repo is unchanged.

The only practical risk is you accidentally giving someone collaborator
access who then makes a mess. Branch protection (above) stops even
collaborators from force-pushing or deleting branches, which is the
worst-case "ruin everything" scenario.
